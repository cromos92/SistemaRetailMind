"""
Módulo de Productos - RetailMind
Contiene todas las vistas relacionadas con productos, inventario, FIFO, lotes y gestión de stock
"""
...
from .models import (
    ...
    Sucursal, EmpresaUser, Empresa, Ticket, Dte
)
...
def verificar_existencia_producto(request):
    """Verificar si existe un producto con características similares"""
    try:
        ...
        queryset = Producto.objects.filter(nombre__icontains=nombre)
        
        if marca_id:
            queryset = queryset.filter(marca_id=marca_id)
        
        productos_similares = queryset[:5]
        ...
        return JsonResponse({
            'success': True,
            'existe': len(productos_similares) > 0,
            'productos_similares': productos_data
        })
        
    except Exception as e:
        return JsonResponse({
            'success': False,
            'error': f'Error al verificar producto: {str(e)}'
        })


def verificar_producto_existente(request):
    """Verificar si un producto ya existe (alias para compatibilidad)"""
    return verificar_existencia_producto(request)
...
def obtener_siguiente_sku():
    ...
    try:
        ...
        # Fallback: generar SKU basado en timestamp
        import time
...
def obtener_siguiente_sku_view(request):
    """Vista para obtener el siguiente SKU disponible"""
    try:
        ...
        return JsonResponse({
            'success': False,
            'error': f'Error al generar SKU: {str(e)}'
        })
...
def obtener_tallas_post(request):
    ...
    if request.method == 'POST':
        try:
            data = json.loads(request.body)
            guia_talla_id = data.get('guia_talla_id')
            
            if not guia_talla_id:
                return JsonResponse({
                    'success': False,
                    'error': 'ID de guía de talla requerido'
                })
            
            guia_talla = get_object_or_404(GuiaTalla, id=guia_talla_id)
            
            tallas_data = []
            for item in guia_talla.guiatallaitems.all().order_by('orden'):
                tallas_data.append({
                    'id': item.talla.id,
...
def _next_sku():
    """Función auxiliar para generar siguiente SKU"""
    return obtener_siguiente_sku()


def _buscar_producto(request, articulo, attr1, attr2, attr3):
    """Función auxiliar para buscar producto por atributos"""
    try:
        # Construir filtros de búsqueda
        filtros = Q(nombre__icontains=articulo)
        
        if attr1:
            filtros &= Q(productos_atributos__opcion__nombre__icontains=attr1)
        if attr2:
            filtros &= Q(productos_atributos__opcion__nombre__icontains=attr2)
        if attr3:
            filtros &= Q(productos_atributos__opcion__nombre__icontains=attr3)
...
def sku_para_talla(request):
    """Generar SKU para una talla específica"""
    try:
        producto_id = request.GET.get('producto_id')
        talla_id = request.GET.get('talla_id')
        ...
        # Generar SKU base
        sku_base = obtener_siguiente_sku()
        
        # Si hay talla, agregar sufijo
        if talla_id:
            from .models import GuiaTallaItem
            try:
                talla_item = GuiaTallaItem.objects.get(talla_id=talla_id)
                sku = f"{sku_base}-{talla_item.talla.nombre}"
            except GuiaTallaItem.DoesNotExist:
                sku = sku_base
        else:
            sku = sku_base
...
def crear_lote_producto(producto_talla, cantidad, costo_unitario, sobreprecio_unitario, 
    ...
    """Crear un nuevo lote de producto con método FIFO"""
    try:
        ...
        raise ValidationError(f'Error al crear lote: {str(e)}')
...
def consumir_stock_fifo(producto_talla, cantidad_requerida, responsable, ticket=None, 
    ...
    """Consumir stock usando método FIFO (First In, First Out)"""
    try:
        ...
        for lote in lotes_disponibles:
            ...
            cantidad_a_consumir = min(cantidad_pendiente, lote.cantidad_disponible)
...
def obtener_valor_inventario_fifo(producto_talla):
    """Obtener valor del inventario usando método FIFO"""
    try:
...
def obtener_costo_promedio_fifo(producto_talla):
    """Obtener costo promedio ponderado usando método FIFO"""
    try:
        lotes_activos = LoteProducto.objects.filter(
            producto_talla=producto_talla,
            cantidad_disponible__gt=0,
        ...
        for lote in lotes_activos:
            valor_total += lote.cantidad_disponible * lote.costo_unitario
            cantidad_total += lote.cantidad_disponible
        
        return valor_total / cantidad_total if cantidad_total > 0 else 0
...
def exportar_dashboard_productos(request):
    ...
    try:
        ...
        ws_vendidos = wb.create_sheet("Productos Más Vendidos")
        ws_vendidos.append(['Nombre', 'SKU', 'Cantidad Vendida', 'Ingresos'])
        
        for item in dashboard_data['productos_vendidos']:
            ws_vendidos.append([
                item['nombre'],
                item['sku'],
                item['total_vendido'],
        ...
        ws_bajo_stock = wb.create_sheet("Productos Bajo Stock")
        ws_bajo_stock.append(['SKU', 'Nombre', 'Talla', 'Stock'])
        
        for item in dashboard_data['productos_bajo_stock']:
            ws_bajo_stock.append([
                item['sku'],
                item['nombre'],
                item['talla'],
                item['stock']
