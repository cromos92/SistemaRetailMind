...
Sistema ERP/POS para retail chileno (DTE/SII, Transbank, multi-sucursal). Núcleo web Django + cliente desktop Tauri ("NEXO POS") que consume API JWT. Incluye asistente conversacional con Claude.
...
- **`assistant`** — chat conversacional con Claude (Anthropic) + tracing con Langfuse ([retailmind/assistant/](retailmind/assistant/)): `agent.py`, `prompts.py`, `tools.py`
